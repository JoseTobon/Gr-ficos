...
static void init (void)
{
    ...
    float rCode = (float(rand()) / float((RAND_MAX)) * a * 1.0);
    float gCode = (float(rand()) / float((RAND_MAX)) * a * 1.0);
    float bCode = (float(rand()) / float((RAND_MAX)) * a * 1.0);
    
    glColor3f (rCode, gCode, bCode);  // Set fill color for hexagon to red.
    glBegin (GL_POLYGON);
    for (k = 0; k < n   ; k++) {
	theta = TWO_PI * k / n;
